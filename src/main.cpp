...
uint256 hashGenesisBlock("0x1eff4e94c43909fa1696d129d87cff65a9792ace6ed954ec38ef9dad2497c79a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 5); // eSportsMoney: starting difficulty is low
...
const string strMessageMagic = "eSportsMoney Signed Message:\n";
...
{
    // eSportsMoney: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 1200000 blocks, which will occur approximately every 2.3 years
    nSubsidy >>= (nHeight / 1200000); // eSportsMoney: 1200k blocks in ~2.3 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 60 * 60; // eSportsMoney: 4 hours
static const int64 nTargetSpacing = 60; // eSportsMoney: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // eSportsMoney: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // eSportsMoney: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2e1a37decf630aa84e548a23dd507a2f887540c26c44d1eb9e2ce9c127907df8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "19/10/1972 Spacewar";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 42 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049c2cbe4a9a87297db3d4836b25abaa803a9d456ab8cb1316f16faa767672a68cdbed0bf3c3c37436e735a9282c7ab8aedf3d46d716cb0dc55adc4873679b59b5") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1388923200;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1374654;

        if (fTestNet)
        {
            block.nTime    = 1388923200;
            block.nNonce   = 3634316;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc8818160897b17cb5b8e642a5afd5972f35703f624d62630bf5298041f9a41ee"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb8, 0xdd }; // eSportsMoney: increase each by adding 4 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("eSportsMoneyMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("eSportsMoneyMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static eSportsMoneyMiner(CWallet *pwallet)
{
    printf("eSportsMoneyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("esportsmoney-miner");
    ...
    try { loop {
        ...
        printf("Running eSportsMoneyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("eSportsMoneyMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&eSportsMoneyMiner, pwallet));
}
